/*
 * LLVM Intermediate Representation code generator for Juc
 * Generates LLVM IR from abstract syntax trees
 * Supports arithmetic, relational, and logical expressions
 * Handles type conversions and function calls
 */
...
/* LLVM value types for Juc types */
typedef enum { VT_I32, VT_DBL, VT_I1, VT_STR, VT_SARR, VT_VOID } ValTy;

/* String constant structure for LLVM IR generation */
...
/* Variable slot for tracking variables and their storage */
...
typedef struct {
    ...
    int label;
    StrConst *strings;
    VarSlot *vars;          /* function-local variable map */
    char cur_block[64];
    bool terminated;        /* true if current block already has a terminator */
} CG;

/* Convert Juc value type to LLVM type string */
...
static RVal gen_expr(CG *cg, Node *e) {
    ...
    if (strcmp(op, "Mod") == 0) {
        char *res = new_reg(cg);
        if (a.ty == VT_DBL || b.ty == VT_DBL) {
            RVal lhs = (a.ty == VT_I32) ? gen_cast(cg, a, VT_DBL) : a;
            RVal rhs = (b.ty == VT_I32) ? gen_cast(cg, b, VT_DBL) : b;
            cg_emit(cg, "  %s = frem double %s, %s\n", res, lhs.reg, rhs.reg);
            if (lhs.reg != a.reg) free(lhs.reg);
            if (rhs.reg != b.reg) free(rhs.reg);
            free(a.reg); free(b.reg);
            return make_rval(VT_DBL, res);
        } else {
            cg_emit(cg, "  %s = srem i32 %s, %s\n", res, a.reg, b.reg);
            free(a.reg); free(b.reg);
